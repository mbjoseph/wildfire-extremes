...
echo $(data-dir)/ecoregion_summaries.csv
echo $(data-dir)/housing_density.csv
echo $(data-dir)/ecoregion_tri.csv
endef

data: $(processed-data)

$(data-dir)/ecoregion_summaries.csv: R/aggregate-climate-data.R R/get-ecoregion-summaries.R R/fetch-fire-data.R
	Rscript --vanilla R/fetch-fire-data.R
	Rscript --vanilla R/aggregate-climate-data.R
	Rscript --vanilla R/get-ecoregion-summaries.R

$(data-dir)/housing_density.csv: R/summarize-housing-density.R R/helpers.R
...
$(data-dir)/ecoregion_tri.csv: R/compute-terrain-metrics.R
	Rscript --vanilla R/compute-terrain-metrics.R

s3data: $(processed-data)
	aws s3 cp $(data-dir)/ecoregion_summaries.csv s3://earthlab-gridmet/ecoregion_summaries.csv --acl public-read
	aws s3 cp $(data-dir)/housing_density.csv s3://earthlab-mjoseph/demo_evt/housing_density.csv --acl public-read
	aws s3 cp $(data-dir)/ecoregion_tri.csv s3://earthlab-gridmet/ecoregion_tri.csv --acl public-read
