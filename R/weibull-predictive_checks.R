...
ppred_df <- read_rds('weib_ppred_df.rds') %>%
...
total_ppred_df %>%
  ...
  coord_equal()
ggsave(filename = 'fig/weib-ppc-burn-area.pdf', width = 20, height = 6)
...
max_ppred_df %>%
  ...
  theme(legend.position = 'none') +
  coord_equal()
ggsave(filename = 'fig/weib-ppc-burn-max.pdf', width = 20, height = 6)
