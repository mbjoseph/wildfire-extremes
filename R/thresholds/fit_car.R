...
sim_df <- data.frame(us_l4name = rownames(W),
                     phi = phi)
sim_df$area <- er_df %>%
  dplyr::select(us_l4name, area) %>%
  group_by(us_l4name) %>%
...
sim_df$y <- rpois(n, exp(-20 + sim_df$phi + log(sim_df$area)))
hist(sim_df$y)
...
sp_fit <- stan(fit = sp_init, data = stan_d, iter = 2000, chains = 2,
               pars = c('beta', 'phi', 'tau', 'alpha'))
sp_fit
...
stan_d <- list(n = n,
               ...
               D_sparse = D_sparse,
               lambda = lambda)
sp_init <- stan('stan/sparse_icar.stan', data = stan_d, iter = 1, chains = 1)
sp_fit <- stan(fit = sp_init, data = stan_d, iter = 1000, chains = 2,
               pars = c('log_mu', 'beta', 'phi', 'tau'), thin = 1)
sp_fit
traceplot(sp_fit, pars = c('tau', 'beta', 'phi[1]', 'phi[2]', 'phi[3]', 'lp__'))
pairs(sp_fit, pars = c('tau', 'beta', 'phi[1]', 'phi[2]', 'phi[3]', 'lp__'))
plot(sp_fit, pars = 'phi') + coord_flip()
