...
model {
  ...
  // fire sizes
  sizes ~ frechet(shape, mu_burn[burn_idx]);
}
...
generated quantities {
  ...
  for (i in 1:n_fire) {
    loglik_f[i] = frechet_lpdf(sizes[i] | shape, mu_burn[burn_idx[i]]);
  }

  mu_full = exp(alpha[1] + csr_matrix_times_vector(N * T, p, w, v, u, beta[1, ]'));
  for (i in 1:n_holdout_b) {
    holdout_loglik_b[i] = frechet_lpdf(holdout_b[i] | shape, mu_full[holdout_b_idx[i]]);
  }
