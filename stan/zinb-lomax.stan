data {
  ...
  int<lower = 1, upper = 5> M; // num dimensions
...
model {
  ...
  // fire sizes
  sizes ~ pareto_type_2(size_threshold,
                        exp(mu_burn[1])[burn_idx],
                        exp(mu_burn[2])[burn_idx]);
}
...
generated quantities {
  ...
  for (i in 1:n_fire) {
    loglik_f[i] = pareto_type_2_lpdf(sizes[i] | size_threshold,
                                                exp(mu_burn[1][burn_idx[i]]),
                                                exp(mu_burn[2][burn_idx[i]]));
  }
