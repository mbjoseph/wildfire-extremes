...
transformed parameters {
  ...
  phi_y[, 1] = y0 + phi_yR[1] * sigma_phi_y[1];
  phi_z[, 1] = z0 + phi_zR[1] * sigma_phi_z[1];

  {
    vector[J] delta_y;
    for (i in 2:n_year) {
      delta_y = sigma_phi_y[i] * phi_yR[i];
      phi_y[, i] = gamma[1] * phi_y[, i - 1] + delta_y;
...
generated quantities {
  ...
  for (i in 1:n_year) {
    for (j in 1:J) {
      ...
      mu_z_new[j, i] = phi_z[j, i];
      y_new[j, i] = poisson_log_rng(log_lambda_new[j, i]);
      if (y_new[j, i] > 0) {
      { // simulate fire sizes
        vector[y_new[j, i]] z_new;
        for (k in 1:y_new[j, i]) z_new[k] = normal_rng(mu_z_new[j, i], sigma_z);
