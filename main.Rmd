...
We construct a predictive model by exploiting spatiotemporal dependence and allowing drivers of fire risk to have nonlinear spatially varying effects, with 98.9\% interval coverage for the number of fires over 1000 acres in a withheld data set over a five-year prediction time horizon. 
...
Useful medium to long term predictions for wildfire extremes might contain information on their occurrence (where and when), and magnitude (burn area) over a monthly time scale. 
...
We also excluded data from Alaska, Hawaii, and Puerto Rico, so that our study area is a continuous swath of land, comprising the lower 48 states, resulting in 16,307 fire events under consideration. 
...
The models differed in the distributions used in the likelihood, with each model representing counts in each month as a Poisson, negative binomial, zero-inflated Poisson, or zero-inflated negative binomial random variable (Table \ref{tab:count-likelihoods}). 
...
We assumed that count values across ecoregions and time steps were conditionally independent, so that the joint probability of all counts was given by: 

$$[\matr{N} | \theta_f] = \prod_{s = 1}^S \prod_{t = 1}^T [n_{s, t} | \theta_f],$$

where $\matr{N}$ is an $S \times T$ array of integers and $n_{s, t}$ is the element in row $s$, column $t$.
...
Concatenating over spatial and temporal units, so that $\bm{\mu} = (\mu_{s=1, t=1}, \mu_{s=2, t=1}, ..., \mu_{s=S, t=1}, \mu_{s=S, t=2}, ..., \mu_{s=S, t=T})$, and similarly for $\bm{\pi}$, we modeled distributional mean and zero inflation parameters as: 
...
\subsubsection*{Burn area}

We developed multiple candidate models for burn area, each of which specified a different distribution for burn areas [@reed2002power; @hernandez2015statistical], including the generalized Pareto [@hosking1987parameter], tapered Pareto [@schoenberg2003distribution], lognormal, gamma, and Weibull distributions (Table \ref{tab:area-likelihoods}). 
...
Fire events were assumed to be conditionally independent, so that the joint probability density of all fire events $i = 1, ..., n_{\textnormal{tot}}$, where $n_{\textnormal{tot}} = \sum_{s}\sum_{t} n_{s, t}$,  was defined as the product of event-wise burn area likelihoods $[y_i | \theta_g]$:

$$[\bm y | \theta_g] = \prod_{i = 1} ^ {n_{\textnormal{tot}}} [y_i | \theta_g].$$
...
\subsubsection*{Accounting for nonlinear forcing}
...
The hierarchical nature of ecoregion designations lends itself to such interactions. 
...
In total $\matr{X}$ has $p=3474$ columns.  
<!-- TODO: update p here now that the tensor product smooths are removed -->

\subsection*{Prior specification}

\subsubsection*{Wildfire occurrence} 
...
\subsection*{Posterior predictive inference and extremes}
...
The best performing count model and all burn area models were fit using the No-U-Turn Sampler, with a maximum treedepth of 11 and a delta adaptation rate of 0.99 [@hoffman2014no].
...
\subsection*{Wildfire occurrence}

The zero-inflated negative binomial distribution performed best on the held-out test set (Figure \ref{loglik-counts}), and was able to recover the proportion of zeros, count maxima, and count totals in posterior predictive checks for both the training and test data (Figure \ref{ppc-counts}). 
...
<!-- % * <jabatzoglouui@gmail.com> 2018-03-03T01:09:43.131Z: -->
<!-- %  -->
<!-- % Seems like a map that shows the regions of interest is needed. I don't understand why some of these say the region name and others just variables. It looks like most of the data points are these very light grey circles that are not visible unless the figure is very large. You might consider better emphasizing these, having them a different color than the grid as well. -->
<!-- %  -->
<!-- % ^. -->
\begin{figure}[ht]
\includegraphics[width=\textwidth]{fig/bivar-effs.png}
\caption{\color{Gray} Posterior median fire count coefficients. The x-axis represents the effect on the logit complementary probability of a structural zero, and the y-axis shows the effect on the negative binomial log mean. Effects with an 85\% or greater posterior probability of being positive or negative are labeled. B-spline vectors are indicated by colons. Humidity:1 indicates the first basis vector corresponding to very low humidity. Interactions are labeled with a \code{x} between variables. Ecoregions are labeled by numeric level and name. Note the cluster of points close to zero resulting from the shrinkage prior.} 
\label{bivar-effs}
\end{figure}

Humidity and air temperature had the strongest effects in the count model (Figure \ref{bivar-effs}). 
<!-- % * <jabatzoglouui@gmail.com> 2018-03-03T01:12:57.971Z: -->
<!-- %  -->
<!-- % Is this minimum RH? The x-axis should go from 0-100, rather than expressed as a fraction -->
<!-- %  -->
<!-- % ^. -->
<!-- % * <jabatzoglouui@gmail.com> 2018-03-03T00:59:08.250Z: -->
<!-- %  -->
<!-- % Very likely, this is catching the seasonal cycle of fires, they burn when temperatures and high and RH is low; the question is how well this accounts for some of the year-to-year variability; I'm also confused whether you build one model per ecoreion and/or how that is being visualized here -->
<!-- %  -->
<!-- % ^. -->
...
Temperature, housing density, wind speed, and long term precipitation were also important, and the model uncovered unique effects of these variables at level 1, 2, and 3 ecoregions (Figure \ref{bivar-effs}, \ref{fire-effs}).
<!-- % * <jabatzoglouui@gmail.com> 2018-03-03T01:14:26.048Z: -->
<!-- %  -->
<!-- % RH looks to be the strongest variable for partial effects; I would think that some measure of fuel moisture might be even better -->
<!-- %  -->
<!-- % ^. -->
...
In the L1 Great Plains, the reduction in expected count from cold temperatures was relatively weak (Figure \ref{bivar-effs}).
...
Last, very hot conditions in the L3 Sonoran Desert had weaker effects on expected counts, relative to the rest of the contiguous US (Figure \ref{bivar-effs}). 
...
This effect was different in the L3 Sonoran Desert (Figure \ref{bivar-effs}), where very dry conditions were actually associated with a reduction in expected count. 
High winds were also associated with higher expected counts (Figure \ref{bivar-effs}).
...
\subsection*{Wildfire burned areas}

The log normal distribution performed best on the test set (Figure \ref{loglik-burns}), and captured tail-behaviour (Figure \ref{ppc-density-funs}).
...
The gamma and Weibull models performed very poorly on the test set, apparently due to a lack of congruence between the shapes of these distributions and the actual burn area distribution (Figures \ref{loglik-burns} and \ref{ppc-density-funs}).
Hereafter we present results for the log normal model.
...
\subsection*{Inference on extremes}
...
\subsection*{Error analysis case study: the 2011 Wallow Fire}
...
Normal distributions are parameterized with mean and variance parameters.
ZIP and ZINB are probability mass functions in Table \ref{tab:count-likelihoods}.
...
\begin{figure}[ht] 
\includegraphics[width=\textwidth]{fig/loglik-counts.png}
\caption{\color{Gray} Posterior performance comparisons for wildfire count models. Log likelihood on the training set is shown on the x-axis, with higher values indicating higher model performance, and the log likelihood of the withheld test set is shown on the y-axis. Each point represents a draw from the approximate posterior distribution.}
\label{loglik-counts} 
\end{figure}
...
\begin{figure}[ht] 
\includegraphics[width=\textwidth]{fig/loglik-burns.png}
\caption{\color{Gray} Posterior performance comparisons for burn area models. Log likelihood on the training set is shown on the x-axis, with higher values indicating higher model performance, and the log likelihood of the withheld test set is shown on the y-axis. Each point represents a draw from the posterior distribution. The gamma and Weibull models performed so poorly on the test set that they are not included on this plot.}
\label{loglik-burns} 
\end{figure}
