...
For each model, we provide unnormalized posterior density specifications, including priors, in \nameref{joint-distributions}. 
...
Spatiotemporal random effects were constructed in the same way as for the count models, and full prior specifications with unnormalized posterior densities for burn area models are provided in  \nameref{joint-distributions}.
...
The lowest test set coverage was `r 100 * round(most_worst$coverage)`\%, for the `r most_worst$NA_L3NAME` L3 ecoregion, followed by `r 100 * round(second_worst$coverage, 1)`\%, for the `r second_worst$NA_L3NAME` L3 ecoregion, though these ecoregions had just `r most_worst$n` and `r second_worst$n` wildfire events in the test period. 
Overall, among test set burn areas that were not contained within the 95\% prediction interval, `r 100 * round(smaller_larger_df$p_smaller, 3)`\% of wildfires were smaller than predicted, and `r 100 * round(1 - smaller_larger_df$p_smaller, 3)`\% of wildfires were larger than predicted. 
...
We investigate this discrepancy further in the error analysis case study below.
